...
export async function POST(req: NextRequest) {
  ...
  const attachments = await resolveBusinessAttachments(request.businessId, parsed.data.attachments.map(({ mediaId }) => mediaId));
  if (!attachments) return NextResponse.json({ error: "invalid_attachment" }, { status: 400 });
  const [message] = await db.insert(serviceRequestMessages).values({ id: crypto.randomUUID(), requestId: request.id, authorUserId: session.user.id, body: parsed.data.body, attachments, isInternal: false }).returning();
